...
/**
 * @description 문서를 api로 추가하는 함수 두 번째 인수 ID를 삽입하여 하위 요소로 넣을 수 있음. ID를 삽입하지 않으면 루트에 들어감.
 * @param {string} title
 * @param {number} [parentDocId]
 * @returns Promise data(JSON)
 */
export async function setDoc(title, parentDocId = null) {
  let options = {
    method: 'POST',
    body: JSON.stringify({ title, parent: parentDocId }),
  }
  let submitOptions = { ...defaultOption, ...options }
...
// setDoc('test', 'test')
// putDoc('타이틀이에요', '추가된 컨텐츠에요', 153941);